...
public:
    ...
    //使用了类型转换符 static_cast，将整型字面量 -1 转换为 size_type 类型，然后进行取反操作，得到一个无符号整数的最大值。
    //这个值用于计算容器能够容纳的最大元素数量。
...
//分配新的内存空间并将所有元素移动过去，设置begin, end, cap的值                                                                                
template<class T>
...
/*
    这段代码是vector类模板的emplace函数的实现，用于在容器中的指定位置插入一个新元素。

    首先进行参数的校验，确保pos指向的位置在容器的范围内。

    然后定义一个迭代器xpos来表示pos所指向的位置。计算出n为pos相对于容器起始位置的偏移量。

    如果容器还有空间，且插入位置是末尾，则直接在末尾构造一个新元素。否则，使用拷贝构造函数将最后一个元素复制到末尾，然后将[pos,end-1)区间内的元素向后移动一位，并在pos位置构造一个新元素。

    如果容器已没有空间，则调用reallocate_emplace函数重新分配空间，并在pos位置构造一个新元素。
...
// 重置容器大小
// 如果n大于容器大小，向容器中插入一些其他值 如果n小于容器大小，删除多余的元素
template <class T>
...
// get_new_cap 函数 根据需要增加的容量计算实际的扩容大小
// 首先保证原容量加上需要增加的容量不会超过最大容量
// 如果二倍扩容大于最大容量，根据需要的大小扩容
// 不然，则根据二倍扩容和所需的扩容空间扩容
template <class T>
...
// copy_assign 函数
/*
    copy_assign函数根据要替换的数据的迭代器类型选择使用input_iterator_tag还是forward_iterator_tag。
    两者的区别在于，前者只能逐个遍历迭代器；后者可以随机访问迭代器、并通过确定的偏移量快速定位到指定位置。

    当使用input_iterator_tag时，先通过迭代器逐个遍历容器内的元素，并替换为相应的外部数据。
    当外部数据迭代器first到达区间尾部或容器内元素迭代器cur到达容器尾部时，结束循环。
    如果外部数据已经遍历结束，则调用erase函数删除容器内剩余的元素。否则，调用insert函数将剩余的外部数据插入到容器末尾。

    当使用forward_iterator_tag时，首先计算出外部数据的长度，如果长度超过容器的容量，
    则使用临时vector容器tmp来存储外部数据，并使用swap函数来交换容器内部的数据和tmp中的数据。
    这样能够快速扩大容器的容量，从而容纳更多的数据。
    如果容器已经足够放置外部数据，则使用mystl::copy将外部数据复制到容器内，并洗刷掉多余的元素。
    如果容器内的元素不够，则先将前半段外部数据复制到容器内，
    然后调用mystl::uninitialized_copy将容器外剩余的数据复制到容器内并在未初始化的内存上构造新元素。
    最后更新容器的end_指针，使其指向最后一个元素的下一个位置。
*/
template <class T>
template <class IIter>
void vector<T>::
...
// 用 [first, last) 为容器赋值 类似于fill assign
template <class T>
template <class FIter>
void vector<T>::
...
// copy_insert 函数
/*
    这段代码是vector类模板的copy_insert函数的实现，用于在容器中的指定位置插入一段新元素。

    首先判断要插入的数据范围[first, last)是否为空，如果是，则直接返回。

    然后计算要插入的数据的个数n。如果备用空间的大小足够，在pos位置后面插入新元素。
    如果插入位置后的剩余空间足够容纳n个元素，则采用三次拷贝操作来将数据插入：
    先使用mystl::uninitialized_copy在末尾处从后往前复制n个元素，
    然后使用mystl::move_backward将[pos, old_end-n)范围内的元素向后移动n个位置，
    最后使用mystl::uninitialized_copy将外部数据复制到[pos, pos+n)范围内。

    如果备用空间不足，则需要重新分配内存，先计算出新的容器大小，
    然后使用data_allocator::allocate分配新的内存。
    接着使用mystl::uninitialized_move将[begin_, pos)范围内的元素移动到新的内存中，
    使用mystl::uninitialized_copy将外部数据复制到新的内存中，
    最后使用mystl::uninitialized_move将[pos, end_)范围内的元素移动到新的内存中。
    如果在操作过程中抛出了异常，则使用destroy_and_recover函数销毁申请到的新内存，并重新抛出异常。

    最后释放旧的内存并更新begin_、 end_、cap_等成员变量的值。
*/
template <class T>
template <class IIter>
void vector<T>::
